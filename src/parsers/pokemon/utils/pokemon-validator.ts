...
class PokemonValidator {
    ...
    static isShadowPokemon(pokemon: BasePokemon): boolean {
        return pokemon.tags?.includes('shadow') ?? pokemon.speciesName.toLowerCase().includes('(shadow)');
    }
    ...
    static isMegaPokemon(pokemon: BasePokemon): boolean {
        return pokemon.tags?.includes('mega') ?? pokemon.speciesName.toLowerCase().includes('(mega)');
    }
