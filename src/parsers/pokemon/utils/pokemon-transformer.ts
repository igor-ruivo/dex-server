...
class PokemonTransformer {
	...
	static getGoForm(pokemonName: string): string {
		...
		if (
			pokemonName.length - pokemonName.replaceAll('(', '').length === 1 &&
			!pokemonName.includes('Shadow') &&
			!pokemonName.includes('Jr')
		) {
			const form = pokemonName.substring(
				pokemonName.indexOf('(') + 1,
				pokemonName.indexOf(')')
			);
			if (form.includes(' ')) {
				console.log(
					'Warning: form for pokémon go asset containing spaces: ' +
						pokemonName +
						' -> form is: ' +
						form
				);
		...
		if (
			...
			console.log(
				'Missing form conversion for pokémon go asset ' + pokemonName
			);
		}
