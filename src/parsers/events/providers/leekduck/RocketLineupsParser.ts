...
export class RocketLineupsParser {
    async parse(gameMasterPokemon: Record<string, GameMasterPokemon>, translator: GameMasterTranslator): Promise<Array<IRocketGrunt>> {
        ...
        for (const entry of entries) {
            const trainerId = (entry.getElementsByClassName('name')[0] as HTMLElement)?.textContent?.replaceAll(/&nbsp;|\u00A0/g, " ").trim() ?? '';
            const typeIdx = trainerId.indexOf('-type');
            const type = typeIdx !== -1 ? trainerId.substring(0, typeIdx).toLocaleLowerCase() : undefined;
            const tier1 = Array.from(entry.getElementsByClassName('lineup-info')[0].children[0].getElementsByClassName('shadow-pokemon')).map(p => (p as HTMLElement)?.getAttribute('data-pokemon')?.trim() ?? '');
            const tier2 = Array.from(entry.getElementsByClassName('lineup-info')[0].children[1].getElementsByClassName('shadow-pokemon')).map(p => (p as HTMLElement)?.getAttribute('data-pokemon')?.trim() ?? '');
            const tier3 = Array.from(entry.getElementsByClassName('lineup-info')[0].children[2].getElementsByClassName('shadow-pokemon')).map(p => (p as HTMLElement)?.getAttribute('data-pokemon')?.trim() ?? '');
            ...
            const catchableTiers = Array.from(entry.getElementsByClassName('lineup-info')[0].children).map((c: Element, i: number) => c.classList.contains('encounter') ? i : undefined).filter(e => e !== undefined);

            const translatedPhrases: Partial<Record<AvailableLocales, string>> = {};
            Object.values(AvailableLocales)
                .forEach(locale => translatedPhrases[locale] = translator.getTranslationForRocketPhrase(locale, trainerId, type));
