...
import type { IRocketGrunt } from '../../../types/events';
...
class RocketLineupsParser {
	...
	async parse() {
		...
		for (const entry of entries) {
			...
			const type =
				typeIdx !== -1
					? trainerId.substring(0, typeIdx).toLocaleLowerCase()
					: undefined;
			const tier1 = Array.from(
				entry
					.getElementsByClassName('lineup-info')[0]
					.children[0].getElementsByClassName('shadow-pokemon')
			).map(
			...
			const tier2 = Array.from(
				entry
					.getElementsByClassName('lineup-info')[0]
					.children[1].getElementsByClassName('shadow-pokemon')
			).map(
				(p) => (p as HTMLElement)?.getAttribute('data-pokemon')?.trim() ?? ''
			);
			const tier3 = Array.from(
				entry
					.getElementsByClassName('lineup-info')[0]
					.children[2].getElementsByClassName('shadow-pokemon')
			).map(
				(p) => (p as HTMLElement)?.getAttribute('data-pokemon')?.trim() ?? ''
			);
			...
			const catchableTiers = Array.from(
				entry.getElementsByClassName('lineup-info')[0].children
			)
				.map((c: Element, i: number) =>
					c.classList.contains('encounter') ? i : undefined
				)
			...
			Object.values(AvailableLocales).forEach(
				(locale) =>
					(translatedPhrases[locale] =
						this.translator.getTranslationForRocketPhrase(
							locale,
							trainerId,
							type
						))
	...
	private convertToShadowVersion(
		pokemonId: string,
