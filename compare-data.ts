...
async function compareData() {
  ...
  try {
    ...
    const oldPokemon = JSON.parse(oldDataRaw) as PokemonData;
...
function comparePokemonData(oldData: PokemonData, newData: PokemonData): ComparisonResult {
...
function comparePokemon(oldPokemon: Pokemon, newPokemon: Pokemon): PokemonDiff[] {
  const differences: PokemonDiff[] = [];
  
  // Fields to compare
  const fields: (keyof Pokemon)[] = [
...
async function generateReport(result: ComparisonResult) {
  const report = [
