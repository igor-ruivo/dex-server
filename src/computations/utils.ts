import { GameMasterPokemon, IGameMasterMove } from 'src/parsers/types/pokemon';
...
export const getAllFastMoves = (p: GameMasterPokemon, moves: Record<string, IGameMasterMove>) => {
    return Array.from(
        new Set(
            p.fastMoves
...
export const getAllChargedMoves = (p: GameMasterPokemon, moves: Record<string, IGameMasterMove>) => {
    return Array.from(
        new Set(
            p.chargedMoves
                .concat(p.eliteMoves?.filter((m) => !moves[m].isFast) ?? [])
